...
create table if not exists bus_route_stop (
    ...
    route_stop_id char(19) unique generated always as ( stop_id::char(9) || '_' || route_id::char(9) ) stored,
...
create table if not exists bus_realtime(
    route_stop_id varchar(20) not null, -- 노선별 정류장 ID
    ...
    constraint pk_bus_realtime primary key (route_stop_id, arrival_sequence),
    constraint fk_bus_realtime_stop_id
        foreign key (route_stop_id) references bus_route_stop(route_stop_id)
);
